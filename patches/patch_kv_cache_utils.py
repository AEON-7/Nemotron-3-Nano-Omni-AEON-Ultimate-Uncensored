...
def patch_kv_cache_utils() -> None:
    ...
    if old not in src:
        # Upstream changed this code path — likely landed an equivalent fix.
        # Don't raise; print and skip so the build doesn't stop.
        print(f"[{target.name}] anchor not found — assuming upstream merged equivalent; skipping")
        return
