"""Refusal-rate benchmark on the live NVFP4 vLLM endpoint — v3.

Same as v2 but bumps max_new=1200 for BOTH thinking modes to eliminate any chance of
length-truncation artifacts. Saves full generations + (refused, empty) flags to JSON.
"""
import asyncio, aiohttp, json, random, re, time
...
CONCURRENCY = 8
MAX_NEW = 1200
OUT_JSON = "/tmp/nvfp4_refusal_100_v3.json"
...
async def run_mode(session, prompts, thinking, results):
    ...
    print(f"\n=== NVFP4 v3 thinking={thinking} max_new={MAX_NEW}, {len(prompts)} prompts ===")
...
async def main():
    ...
    results = {"meta": {"model": MODEL, "n_prompts": N_SAMPLE, "seed": 42, "max_new": MAX_NEW},
               "modes": {}}
